...
fn render_plot(
    ...
    let mut pixel_buffer =
        slint::SharedPixelBuffer::new(width as u32, (width as f64 / aspect) as u32);
    ...
    chart
        .draw_series(
            (0..naxis1)
                .flat_map(move |i| (0..naxis2).map(move |j| (i, j)))
                .map(|(i, j)| {
                    ...
                    Rectangle::new(
                        [
                            (i as i32, (naxis2 - j) as i32),
                            ((i + 1) as i32, (naxis2 - j + 1) as i32),
                        ],
                        RGBAColor(c.0, c.1, c.2, (c.3 as f64) / 255.).filled(),
                    )
                }),
        )
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    let stations: Vec<slint::StandardListViewItem> = ants
        ...
        .collect();
    let stations_model = std::rc::Rc::new(slint::VecModel::from(stations.clone()));
    ...
    ui.on_plot({
        ...
        move || {
            ...
            ui2.on_decrease_ant({
                ...
                move || {
                    let new_idx = (handle.get_idx_ant() - 1) as usize;
